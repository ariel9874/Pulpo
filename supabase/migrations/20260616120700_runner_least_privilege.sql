-- Pulpo · Etapa 21 — Mínimo privilegio del token del runner
--
-- El token que mintea el pairing es un JWT de rol `authenticated` del usuario,
-- pero con un claim extra `pulpo_machine_id` (plantado en la Etapa 7 justo para
...
--
-- Modelo: si el JWT trae `pulpo_machine_id` (es un runner) → solo ve/escribe lo
...
returns uuid language sql stable as $$
  select nullif(auth.jwt() ->> 'pulpo_machine_id', '')::uuid
$$;
